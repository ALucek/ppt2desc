...
async def convert_presentation(
    ...
    try:
        ...
        ppt_file, image_paths = results[0]

        # The actual slide descriptions are written to a JSON file
        # Read the JSON file that was generated by the processor
        ...
        if not json_file_path.exists():
            raise HTTPException(
                status_code=500,
                detail=f"Processing completed but JSON file not found: {json_filename}"
            )
        ...
        try:
            ...
            if "slides" not in json_data:
                raise HTTPException(
                    status_code=500,
                    detail="Invalid JSON structure - missing 'slides' key"
                )

            # Return the JSON data directly (it's already in the correct format)
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Failed to parse generated JSON: {str(e)}"
            )
